...
public class UserDao {
   public boolean  add(User user){
       ...
       if(con!=null){
        try{
            String sql = "Insert into user(name,fname,gender,dob,hobbies,user_id,password,photo,contact) values(?,?,?,?,?,?,?,?,?)";
   ...
   public boolean  update(User user){
       ...
       if(con!=null){
        try{
            String sql = "update User set name=?,fname=?,gender=?,dob=?,hobbies=?,user_id=?,password=?,photo=?,contact=? where id=?";
            PreparedStatement smt = con.prepareStatement(sql);
            smt.setString(1, user.getName());
            smt.setString(2, user.getFname());
            smt.setString(3, user.getGender());
    ...
    public boolean  isContact(String user_id){
       boolean status = false;
        ConnectionPool cp = ConnectionPool.getInstance();
       cp.initialize();
       Connection con = cp.getConnection();
